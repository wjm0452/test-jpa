...
public class CodeTests {
    ...
    void largeCodes() {
        ...
        assertThat(codeService.getLargeCodes()).isNotEmpty();

        // ignore case
        LrgclasCd largeCode = LrgclasCd.builder().lrgclasCd("org_KIND_DVCD").build();
        assertThat(codeService.getLargeCodes(largeCode)).size().isEqualTo(1);

        // not contain
        largeCode = LrgclasCd.builder().lrgclasCdNm("실제없어야 테이블에 합니다.").build();
        assertThat(codeService.getLargeCodes(largeCode)).size().isEqualTo(0);
        ...
        assertThat(codeService.getLargeCodes(largeCode)).size().isGreaterThan(0);
    ...
    @Test
    void largeCode() {
        assertThat(codeService.getLargeCode("ORG_KIND_DVCD").getLrgclasCd()).isEqualTo("ORG_KIND_DVCD");
        assertThat(codeService.getLargeCode("ORG_KIND_DVCD2")).isNull();
    }

    @Test
    void smallCodes() {
        // large code
        assertThat(codeService.getSmallCodes("ORG_KIND_DVCD")).size().isGreaterThan(0);

        // large code
        assertThat(codeService.getSmallCodes("ORG_KIND_DVCD2")).size().isEqualTo(0);
    }

    @Test
    void smallCode() {
        // small code
        assertThat(codeService.getSmallCode("ORG_KIND_DVCD", "CENTER")).isNotNull();
    ...
    void largeAndSmallCode() {
        ...
        LrgclasCd lrgclasCd = codeService.getLargeCode("ORG_KIND_DVCD");
    ...
    @Test
    void multiCodes() {
        List<LrgclasCd> lrgclasCds = codeService.getCodes("ORG_KIND_DVCD", "MSG_CD");
